              ...
              resultsList.sort((a:any,b:any) => {return reverse ?  b.id - a.id : a.id - b.id}).map((answer:any) => (
                  <Result key={answer.question}>
                      <Subtitle >{replaceSpecialCharacters( answer?.question) }</Subtitle>
                      {
                          ...
                          : (
                              <>
                                <p className="aligned incorrect-icon"><i style={{color:'red',fontSize:'24px'}} className='bx bxs-x-circle'></i>  <span style={{color:'red'}}> { replaceSpecialCharacters(answer.selectedAnswer)}</span></p>
